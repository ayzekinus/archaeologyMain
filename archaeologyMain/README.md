...
**PostgreSQL 18+ veri dizini hatası alıyorum**

Yeni Postgres imajlarında (18+), veri dizini `/var/lib/postgresql` altına taşındı.
Bu yüzden volume mount noktası `/var/lib/postgresql/data` yerine
`/var/lib/postgresql` olmalıdır. `docker-compose.yml` bu düzene göre güncellendi.

Eğer eski bir volume kullanıyorsanız, temiz başlatmak için aşağıdaki komutları
kullanabilirsiniz (veritabanı verileri silinir):
```bash
